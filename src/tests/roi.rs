...
fn worldwide_map() {
    ...
    assert!(ionex.is_worldwide_map(), "Worldwide map from standardized file name!");
    ...
    let reduced = ionex.to_regional_ionex(roi.into())
        .unwrap();

    assert!(reduced.is_regional_map(), "Worldwide map reduced to Regional ROI"); 
